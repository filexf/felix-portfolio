...
export default function LanguageSwitcher({ currentLanguage = 'en', onLanguageChange }) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button 
        className={`flex items-center justify-center gap-2 rounded-full border ${isOpen ? 'ring-2 ring-opacity-50' : ''} px-3 py-1.5 transition-all duration-300 hover:shadow-md`}
      ...
      >
        <span className="text-lg mr-0.5">{languages[currentLanguage].flag}</span>
        <span className="text-sm font-semibold">{languages[currentLanguage].code}</span>
        <svg 
      ...
      {isOpen && (
        <div 
          className="absolute right-0 mt-1.5 w-28 origin-top-right rounded-lg shadow-lg overflow-hidden z-50 transform transition-all duration-200 ease-out"
          style={{ 
              ...
              <button
                ...
                className={`flex w-full items-center px-3 py-2.5 text-sm transition-colors duration-150 ${
                  lang === currentLanguage 
                    ? 'font-semibold bg-opacity-10 bg-slate-400' 
